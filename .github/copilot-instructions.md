...
Purpose: Interactive R Shiny app to explore human pancreatic follicle measurements by donor group and follicle size, plus a lightweight embedded image viewer (Avivator/Viv) for local OME-TIFFs.
...
- Stats tab: Global model is `value ~ donor_status + follicle_diam_um`; pairwise t-tests on residuals from `value ~ follicle_diam_um` with BH adjustment. Respect factor order ND < Aab+ < T1D.
